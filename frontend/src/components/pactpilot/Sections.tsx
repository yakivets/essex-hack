    ...
    <>
        ...
        <Section
          ...
          badge={
            <span className="text-xs text-muted-foreground tabular">
              {data.obligations.yours.length} yours · {data.obligations.theirs.length} theirs
            </span>
          }
        >
        ...
        >
          <div className="grid grid-cols-2 gap-3 pt-3">
            ...
            <Tile
              label="Penalties"
              value={
                data.money.penalties?.length ? `${data.money.penalties.length} terms` : undefined
              }
            />
          </div>
          {data.money.penalties?.length ? (
            <ul className="mt-4 space-y-1.5 text-sm text-muted-foreground">
              {data.money.penalties.map((p, i) => (
                <li key={i}>• {p}</li>
              ))}
            </ul>
          ) : null}
      ...
      {data.dates?.length ? (
        <Section
          ...
          title="Key dates"
          badge={
            <span className="text-xs text-muted-foreground tabular">{data.dates.length} dates</span>
          }
        >
          <Timeline data={data} />
        </Section>
        ...
        <Section
          ...
          badge={
            <span
              ...
              style={{
                color:
                  data.exit.difficulty === "hard"
                    ? "var(--risk-high)"
                    : data.exit.difficulty === "moderate"
                      ? "var(--risk-med)"
                      : "var(--risk-low)",
              }}
            >
              {data.exit.difficulty}
            </span>
          }
        >
          <div className="pt-3 text-sm">
            ...
            <ul className="mt-3 space-y-1.5 text-muted-foreground">
              {data.exit.termination_terms.map((t, i) => (
                <li key={i}>• {t}</li>
              ))}
            </ul>
          </div>
      ...
      {data.missing_clauses?.length ? (
        ...
        >
          <p className="text-sm text-muted-foreground pt-3 mb-3">
            Protections this contract lacks.
          </p>
      ...
      {data.scenarios?.length ? (
        <Section
          ...
          badge={
            <span className="text-xs text-muted-foreground tabular">{data.scenarios.length}</span>
          }
        >
  ...
  return (
    <div className="pt-6 pb-2">
      <div className="relative h-1.5 rounded-full bg-muted">
        {today >= min && today <= max && (
          <div
            className="absolute -top-1 bottom-0 w-0.5 bg-primary"
            style={{ left: `${todayPct}%` }}
          >
            <div className="absolute -top-5 -translate-x-1/2 text-[10px] uppercase tracking-wider text-primary font-semibold">
              Today
            </div>
          </div>
        )}
