...
export async function exportPdf(data: AnalysisResult) {
  ...
  if (data.red_flags?.length) {
    ...
    for (const f of data.red_flags) {
      ...
      doc.setFontSize(11);
      doc.setTextColor(...TEXT);
      for (const line of doc.splitTextToSize(`${f.title}  (${riskLabel(f.severity)})`, contentW - 14) as string[]) {
        need(15);
  ...
  for (let i = 1; i <= pages; i++) {
    ...
    doc.setFontSize(8);
    doc.setTextColor(...TERTIARY);
    doc.text("Not legal advice — a first-pass review, not a lawyer. Generated by PactPilot.", margin, pageH - 26);
